...
def runWindsteps():
    ...
    return
def Plot3D(nodes_initial,modified_nodes,Step):
    x, y, z, _ = zip(*nodes_initial)
    x1, y1, z1, _ = zip(*modified_nodes)
    fig = plt.figure(figsize=(16, 12))
    ax = fig.add_subplot(111, projection='3d')
    ax.scatter(x, y, z, c='darkgray', marker='o')
    ax.scatter(x1, y1, z1, c='royalblue', marker='o')

    ax.set_xlabel('X (m)')
    ax.set_ylabel('Y (m)')
    ax.set_zlabel('Z (m)')

    what=['Model_BeamElements','Model_ColumnElements']
    what1=[['Leg167Beam','Leg172Beam','Leg173Beam','Leg174Beam','Pylon','BeamBody','Waist',
            'CrossArmHangers','WindowRight', 'WindowLeft', 'Tip', 'CageRight', 'CageLeft'],
           ['Leg167Column','Leg172Column','Leg173Column','Leg174Column','LegBody']   ]
    for iii in range(len(what)):
        
        for Tower_No in range(iTower):
            workbook = openpyxl.load_workbook(f'{what[iii]}{Tower_No+1}.xlsx')
            for ii in range(len(what1[iii])):
                sheet = workbook[what1[iii][ii]]
                for row in range(2, sheet.max_row + 1):
                    
                    # eleName = sheet.cell(row=row, column=1).value
                    endi=int(sheet.cell(row=row, column=3).value)
                    endj=int(sheet.cell(row=row, column=4).value)
                    tag=int(sheet.cell(row=row, column=1).value)
     
                    endi_coords = op.nodeCoord(endi)
                    endj_coords = op.nodeCoord(endj)
                
                    for tt in modified_nodes:
                        if tt[3]==endi:
                            endi_coordsUpdate=tt[:3]
                    for tt1 in modified_nodes:
                        if tt1[3]==endj:
                            endj_coordsUpdate=tt1[:3]
                    ax.plot([endi_coords[0], endj_coords[0]],
                            [endi_coords[1], endj_coords[1]],
                            [endi_coords[2], endj_coords[2]], c='gray', linestyle='-', linewidth=2)
                    
                    y=-0.02549;                z=-0.02549
                    stressElem= op.eleResponse(tag,'section',str(3),'fiber', str(y), str(z), 'stress')[0]/1e6
                    if iii==0:
                        if stressElem>250. or stressElem<-250.:
                            C='r'
                        elif stressElem> 196. or stressElem< -196.:
                            C='lightcoral'
                        elif stressElem< -96.47:
                            C='darkorange'
                        else:
                            C='royalblue'
                    ax.plot([endi_coordsUpdate[0], endj_coordsUpdate[0]],
                            [endi_coordsUpdate[1], endj_coordsUpdate[1]],
                            [endi_coordsUpdate[2], endj_coordsUpdate[2]], c=C, linestyle='-', linewidth=3)
                    if iii==1:
                        if stressElem>345. or stressElem<-345:
                            C='r'
                        elif stressElem> 276. or stressElem< -276.:
                            C='lightcoral'
                        elif stressElem< -131.55:
                            C='darkorange'
                        else:
                            C='royalblue'
                    ax.plot([endi_coordsUpdate[0], endj_coordsUpdate[0]],
                            [endi_coordsUpdate[1], endj_coordsUpdate[1]],
                            [endi_coordsUpdate[2], endj_coordsUpdate[2]], c=C, linestyle='-', linewidth=3)
    colors = ['r', 'lightcoral', 'darkorange', 'royalblue']
    texts = ['Fy',  '0.8Fy','Fcr', 'Elastic Range']
    legend_handles = [Patch(color=color, label=text) for color, text in zip(colors, texts)]
    plt.legend(handles=legend_handles, loc='lower left')
    plt.savefig(f'Outsteps_{Attempt}/{Step}/3DPlot_Step{Step}.png')
    plt.close()
    return
def plotfig_xy(nodes_initial,modified_nodes,Step):
    x, y, z, _ = zip(*nodes_initial)
    x1, y1, z1, _ = zip(*modified_nodes)
    fig_xy = plt.figure(figsize=(16, 12))
    ax_xy = fig_xy.add_subplot(111)
    ax_xy.scatter(x, y, c='darkgray', marker='o')
    ax_xy.scatter(x1, y1, c='royalblue', marker='o')
            ...
            for ii in range(len(what1[iii])):
                ...
                for row in range(2, sheet.max_row + 1):
                    ...
                    for tt1 in modified_nodes:
                        if tt1[3]==endj:
                            endj_coordsUpdate=tt1[:3]
                    ax_xy.plot([endi_coords[0], endj_coords[0]],
                               [endi_coords[1], endj_coords[1]], c='gray', linestyle='-', linewidth=2)
                    ...
                    if iii==0:
                        if stressElem>250. or stressElem<-250.:
                            ...
                            C='royalblue'
                    ax_xy.plot([endi_coordsUpdate[0], endj_coordsUpdate[0]],
                               [endi_coordsUpdate[1], endj_coordsUpdate[1]], c=C, linestyle='-', linewidth=3)
                    if iii==1:
                        if stressElem>345. or stressElem<-345:
                            C='r'
                        elif stressElem> 276. or stressElem< -276.:
    ...
    legend_handles = [Patch(color=color, label=text) for color, text in zip(colors, texts)]
    plt.legend(handles=legend_handles, loc='lower left')
    plt.savefig(f'Outsteps_{Attempt}/{Step}/XYdirTower{Step}.png')
    plt.close()
    return
def plotfig_xz(nodes_initial,modified_nodes,Step):
    x, y, z, _ = zip(*nodes_initial)
    x1, y1, z1, _ = zip(*modified_nodes)

    fig_xz = plt.figure(figsize=(16, 12))
    ax_xz = fig_xz.add_subplot(111)
    ax_xz.scatter(x, z, c='darkgray', marker='o')
    ax_xz.scatter(x1, z1, c='royalblue', marker='o')
            ...
            for ii in range(len(what1[iii])):
                ...
                for row in range(2, sheet.max_row + 1):
                    ...
                    for tt1 in modified_nodes:
                        if tt1[3]==endj:
                            endj_coordsUpdate=tt1[:3]
                    ax_xz.plot([endi_coords[0], endj_coords[0]],
                               [endi_coords[2], endj_coords[2]], c='gray', linestyle='-', linewidth=2)
                    ...
                    if iii==0:
                        if stressElem>250. or stressElem<-250.:
                            ...
                            C='royalblue'
                    ax_xz.plot([endi_coordsUpdate[0], endj_coordsUpdate[0]],
                               [endi_coordsUpdate[2], endj_coordsUpdate[2]], c=C, linestyle='-', linewidth=3)
                    if iii==1:
                        if stressElem>345. or stressElem<-345:
                            C='r'
    ...
    legend_handles = [Patch(color=color, label=text) for color, text in zip(colors, texts)]
    plt.legend(handles=legend_handles, loc='lower left')
    plt.savefig(f'Outsteps_{Attempt}/{Step}/XZdirTower{Step}.png')
    plt.close()
    return
def plotfig_yz(nodes_initial,modified_nodes,Step):
    x, y, z, _ = zip(*nodes_initial)
    x1, y1, z1, _ = zip(*modified_nodes)

    fig_yz = plt.figure(figsize=(16, 12))
    ax_yz = fig_yz.add_subplot(111)
    ax_yz.scatter(y, z, c='darkgray', marker='o')
    ax_yz.scatter(y1, z1, c='royalblue', marker='o')
            ...
            for ii in range(len(what1[iii])):
                ...
                for row in range(2, sheet.max_row + 1):
                    ...
                    for tt1 in modified_nodes:
                        if tt1[3]==endj:
                            endj_coordsUpdate=tt1[:3]
                    ax_yz.plot([endi_coords[1], endj_coords[1]],
                               [endi_coords[2], endj_coords[2]], c='gray', linestyle='-', linewidth=2)  
                    ...
                    if iii==0:
                        if stressElem>250. or stressElem<-250.:
                            ...
                            C='royalblue'
                    ax_yz.plot([endi_coordsUpdate[1], endj_coordsUpdate[1]],
                               [endi_coordsUpdate[2], endj_coordsUpdate[2]], c=C, linestyle='-', linewidth=3)
    ...
    legend_handles = [Patch(color=color, label=text) for color, text in zip(colors, texts)]
    plt.legend(handles=legend_handles, loc='lower left')
    plt.savefig(f'Outsteps_{Attempt}/{Step}/YZdirTower{Step}.png')
    plt.close()
    return
def AllDisp(Step):
    modified_nodes=[]
    for Tower_No in range(iTower):
        workbook = openpyxl.load_workbook('NodesinTower.xlsx')
        sheet = workbook[f'Tower{Tower_No+1}']
        sheet['E1']='Disp x'
        sheet['F1']='Disp y'
        sheet['G1']='Disp z'
        for row in range(2, sheet.max_row + 1):
            tag=int(sheet[f'A{row}'].value)
            sheet[f'E{row}'], sheet[f'F{row}'], sheet[f'G{row}']= op.nodeDisp(tag)[:3]
            modified_nodes.append(list(np.array(op.nodeDisp(tag)[:3])+np.array(op.nodeCoord(tag)))+[tag])
        workbook.save(f'Outsteps_{Attempt}/NodesinTower{Step}.xlsx')
        workbook.close()
...
def nodesInTower():
    ...
    for Tower_No in range(iTower):
        ...
        for row in range(2, sheet.max_row + 1):
            ...
            Tag = sheet[f'A{row}'].value
            x = sheet[f'B{row}'].value
            y = sheet[f'C{row}'].value
            z = sheet[f'D{row}'].value
            nodes_initial.append([x,y,z,Tag])
         
        workbook.close()
    return nodes_initial
def PlotTowerDisplacement(Step):
    nodes_initial=nodesInTower()
    modified_nodes=AllDisp(Step)
    
    Plot3D(nodes_initial,modified_nodes,Step)
    plotfig_xy(nodes_initial,modified_nodes,Step)
    plotfig_xz(nodes_initial,modified_nodes,Step)
    plotfig_yz(nodes_initial,modified_nodes,Step)
